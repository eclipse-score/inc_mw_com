...
class StateMachineEventsFixture : public LolaProxyEventResources
{
  protected:
    ...
    amp::optional<std::reference_wrapper<TransactionLog>> GetTransactionLog(
        const TransactionLogId& transaction_log_id) noexcept
    {
        auto& transaction_log_set = parent_->GetEventControl(element_fq_id_).data_control.GetTransactionLogSet();
    ...
    bmw::Result<TransactionLogSet::TransactionLogIndex> RegisterTransactionLog(
        const TransactionLogId& transaction_log_id) noexcept
    {
        auto& transaction_log_set = parent_->GetEventControl(element_fq_id_).data_control.GetTransactionLogSet();
...
{
    EXPECT_FALSE(IsProxyTransactionLogIdRegistered(kDummyTransactionLogId));
    amp::ignore = state_machine_.SubscribeEvent(max_num_slots_ + 1U);
    EXPECT_FALSE(IsProxyTransactionLogIdRegistered(kDummyTransactionLogId));
}
