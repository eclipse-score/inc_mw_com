...
class ObjectDestructionNotifier
{
  public:
    ...
    {
        AMP_ASSERT_PRD_MESSAGE(is_destructed == false, "Is destructed flag must be false.");
    };
    ~ObjectDestructionNotifier() { is_destructed_ = true; }
...
class SampleAllocateePtrFixture : public ::testing::Test
{
  public:
    ...
    lola::EventDataControl::SlotIndexType event_data_slot_index_{
        std::numeric_limits<lola::EventDataControl::SlotIndexType>::max()};
...
TEST_F(SampleAllocateePtrFixtureDeathTest, CannotUseArrowOperatorOnBlankPointer)
{
    ...
    // When dereferencing the underlying pointer
    // Then the program will terminate
    EXPECT_DEATH(amp::ignore = unit->value, ".*");
}
...
TEST_F(SampleAllocateePtrFixture, UnderlyingLolaPtrIsFreedOnDestruction)
{
    ...
    lola::EventDataControl::SlotIndexType event_data_slot_index{
        std::numeric_limits<lola::EventDataControl::SlotIndexType>::max()};
    ...
    lola::SampleAllocateePtr<ObjectDestructionNotifier> lola_allocatee_ptr{
        &object_destruction_notifier, event_data_ctrl, event_data_slot_index};
    {
