...
{
  public:
    using StrictFcntlMock = StrictMock<bmw::os::FcntlMock>;
    using StrictUnistdMock = StrictMock<bmw::os::UnistdMock>;

    void SetUp() override
    {
        memory_resource_ = amp::pmr::get_default_resource();
        auto fcntl_mock = amp::pmr::make_unique<StrictFcntlMock>(memory_resource_);
        auto unistd_mock = amp::pmr::make_unique<StrictUnistdMock>(memory_resource_);
...
TEST_F(ResmgrSenderFixture, NormalFlow)
{
    ...
    EXPECT_CALL(*raw_fcntl_mock_, open(StrEq(path.c_str()), bmw::os::Fcntl::Open::kWriteOnly))
