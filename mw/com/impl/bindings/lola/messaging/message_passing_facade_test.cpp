...
{
  public:
    void SetUp() override { message_passing::ReceiverFactory::InjectReceiverMock(&receiver_mock_); }
    ...
    void PrepareFacadeWithListenError()
    {
        ...
        EXPECT_CALL(receiver_mock_, StartListening())
            .WillOnce(Return(amp::make_unexpected(bmw::os::Error::createFromErrno(ARBITRARY_POSIX_ERROR))));

        MessagePassingFacade facade{message_passing_control_mock_, asilCfg, amp::nullopt};
    }
    ...
    void PrepareFacade(bool also_activate_asil)
    {
        ...
        EXPECT_CALL(receiver_mock_, StartListening())
            .Times(numListeners)
            .WillRepeatedly(Return(amp::expected_blank<bmw::os::Error>{}));

        // when creating our MessagePassingFacade
        unit_.emplace(
            message_passing_control_mock_,
            asilCfg,
            also_activate_asil ? amp::optional<MessagePassingFacade::AsilSpecificCfg>{asilCfg} : amp::nullopt);
    }
    ...
    amp::optional<MessagePassingFacade> unit_;
};

TEST_F(MessagePassingFacadeFixture, CreationQMOnly)
{
    ThreadHWConcurrency::injectMock(&concurrency_mock_);
    EXPECT_CALL(concurrency_mock_, hardware_concurrency()).Times(2).WillRepeatedly(Return(0));
    // we have a Facade created for QM only with successful listening call, even with a HW concurrency mock returning 0
    PrepareFacade(false);
...
TEST_F(MessagePassingFacadeFixture, CreationQMOnlyHeap)
{
    ...
    // when creating our MessagePassingFacade
    auto unit_on_heap = std::make_unique<MessagePassingFacade>(message_passing_control_mock_, asilCfg, amp::nullopt);
...
TEST_F(MessagePassingFacadeFixture, RegisterEventNotification)
{
    ...
    auto eventUpdateNotificationHandler =
        BindingEventReceiveHandler(event_receive_handler_scope, []() noexcept { return; });
