...
{
  public:
    using StrictChannelMock = StrictMock<bmw::os::MockChannel>;
    using StrictDispatchMock = StrictMock<bmw::os::MockDispatch>;
    using StrictIoFuncMock = StrictMock<bmw::os::MockIoFunc>;

    void CustomSetUp(const amp::pmr::vector<uid_t>& allowed_uids, bool defensive_test)
    {
        memory_resource_ = amp::pmr::get_default_resource();
        auto channel_mock = amp::pmr::make_unique<StrictChannelMock>(memory_resource_);
        auto dispatch_mock = amp::pmr::make_unique<StrictDispatchMock>(memory_resource_);
        auto iofunc_mock = amp::pmr::make_unique<StrictIoFuncMock>(memory_resource_);
        ...
        EXPECT_CALL(*raw_iofunc_mock_, iofunc_func_init(_RESMGR_CONNECT_NFUNCS, _, _RESMGR_IO_NFUNCS, _))
            ...
            .WillRepeatedly([this](const std::uint32_t,
                                   ...
                                   const std::uint32_t,
                                   resmgr_io_funcs_t* const) { connect_funcs->open = open_default; });
        ...
        if (defensive_test)
        {
            auto test_error = amp::make_unexpected(bmw::os::Error::createFromErrno(EINVAL));
    ...
    void SetUpReceiver(const amp::pmr::vector<uid_t>& allowed_uids)
    {
    ...
    {
        EXPECT_CALL(*raw_dispatch_mock_, dispatch_block(&dispatch_contexts_[kThreadId])).WillOnce(Return(amp::blank{}));
        EXPECT_CALL(*raw_dispatch_mock_, dispatch_handler(&dispatch_contexts_[kThreadId]))
            .WillOnce([this, open_result](auto&& ctp) -> amp::expected_blank<std::int32_t> {
        ...
        EXPECT_CALL(*raw_channel_mock_, ConnectClientInfo(kScoid, _, 0))
            ...
            .WillOnce([this, client_uid](
                          std::int32_t, _client_info* info, std::int32_t) -> amp::expected_blank<bmw::os::Error> {
                if (client_uid == kUidFailInfo)
                {
                    return amp::make_unexpected(bmw::os::Error::createFromErrno(EINVAL));
                }
    ...
    {
        EXPECT_CALL(*raw_dispatch_mock_, dispatch_block(_)).WillRepeatedly(Return(amp::blank{}));
        EXPECT_CALL(*raw_dispatch_mock_, dispatch_handler(_))
            .WillRepeatedly([this, size](auto&& ctp) -> amp::expected_blank<std::int32_t> {
                ...
                {
                    return amp::make_unexpected(-1);
                }
            });
        EXPECT_CALL(*raw_iofunc_mock_, iofunc_write_verify).WillRepeatedly(Return(amp::blank()));
    ...
    {
        EXPECT_CALL(*raw_dispatch_mock_, dispatch_handler(_))
            .WillOnce([this, anomaly, size](auto&& ctp) -> amp::expected_blank<std::int32_t> {
    ...
    void ReceiveNextExpectShortMedium(int short_count, int medium_count)
    {
        const auto next_result = ResmgrReceiverTraits::receive_next(
            fd_,
            kThreadId,
            [this](auto&&) { ++short_message_count_; },
            [this](auto&&) { ++medium_message_count_; },
            os_resources_);
    ...
    bmw::os::UnistdMock* raw_unistd_mock_ = nullptr;

    // consts
    const amp::string_view kIdentifier{"/whatever"};
...
{
  public:
    void SetUp() override { CustomSetUp(kAllowedUids, false); }

    const amp::pmr::vector<uid_t> kAllowedUids = {};
};
...
class ResmgrReceiverFixtureWithUids : public ResmgrReceiverFixtureBase
{
  public:
    ...
    const amp::pmr::vector<uid_t> kAllowedUids = {1001, 1002};
...
{
  public:
    void SetUp() override { CustomSetUp(kAllowedUids, true); }
...
{
    EXPECT_CALL(*raw_dispatch_mock_, dispatch_block(&dispatch_contexts_[kThreadId])).WillOnce(Return(amp::blank{}));
    EXPECT_CALL(*raw_dispatch_mock_, dispatch_handler(&dispatch_contexts_[kThreadId])).WillOnce(Return(amp::blank{}));
...
TEST_F(ResmgrReceiverFixture, ReceiveQueueOverflow)
{
        ...
        if (short_message_count_++ == 0)
        {
            ...
            for (std::uint32_t i = 0; i < 3 * kMaxNumberMessagesInQueue; ++i)
            {
                const auto next_result = ResmgrReceiverTraits::receive_next(
                    fd_,
                    kOtherThreadId,
                    [&racing_message_count](auto&&) { ++racing_message_count; },
                    [&racing_message_count](auto&&) { ++racing_message_count; },
                    os_resources_);
                ASSERT_TRUE(next_result);
                EXPECT_TRUE(next_result.value());
    ...
    const auto next_result = ResmgrReceiverTraits::receive_next(
        fd_, kThreadId, blocking_short_message_processor, [this](auto&&) { ++medium_message_count_; }, os_resources_);
    ASSERT_TRUE(next_result);
    EXPECT_TRUE(next_result.value());
...
TEST_F(ResmgrReceiverFixture, IgnoreDispatchErrors)
{
    ...
    EXPECT_CALL(*raw_dispatch_mock_, dispatch_block(&dispatch_contexts_[kThreadId])).WillOnce(Return(amp::blank{}));
    EXPECT_CALL(*raw_dispatch_mock_, dispatch_handler(&dispatch_contexts_[kThreadId]))
        .WillOnce(Return(amp::make_unexpected(-1)));
...
TEST_F(ResmgrReceiverFixtureDefensive, OpenReceiverDefensiveProgramming)
{
    ...
    EXPECT_CALL(*raw_iofunc_mock_, iofunc_write_verify)
        .WillOnce(Return(amp::make_unexpected(EINVAL)))
        .RetiresOnSaturation();
    ReceiveNextExpectShortMedium(0, 0);
