...
class SkeletonEventAttorney
{
  public:
    ...
    void SetQmDisconnect(const bool qm_disconnect_value) { skeleton_event_.qm_disconnect_ = qm_disconnect_value; }

    amp::optional<EventDataControlComposite>& GetEventDataControlComposite()
    {
...
{
  public:
    void SetUp() override { ON_CALL(runtime_mock_, GetTracingRuntime()).WillByDefault(Return(&tracing_runtime_mock_)); }
    ...
    impl::tracing::ServiceElementInstanceIdentifierView CreateServiceElementInstanceIdentifierView() const noexcept
    {
        ...
        const impl::tracing::ServiceElementInstanceIdentifierView expected_service_element_instance_identifier_view{
            service_element_identifier_view, amp::string_view{instance_specifier_.ToString()}};
        return expected_service_element_instance_identifier_view;
    }

    EventSlotStatus::EventTimeStamp GetLastSendEventTimestamp(const EventDataControl::SlotIndexType slot) noexcept
    {
...
{
    RecordProperty("Verifies", "8, 5, 1, 6, 7, 3");
    RecordProperty("Description",
                   ...
                   "TracePointDataId which is the timestamp of the event slot timestamp (7, 3).");
    ...
    const impl::tracing::ITracingRuntimeBinding::TraceContextId trace_context_id{0U};
...
TEST_F(SkeletonEventTracingSendFixture, MultipleSendCallsUsesCorrectTracePointDataId)
{
    ...
    // Then a trace call relating to Send should be called containing the correct sample data and trace point id
    impl::tracing::ITracingRuntime::TracePointType trace_point_type{impl::tracing::SkeletonEventTracePointType::SEND};
    EventDataControl::SlotIndexType slot_index{0U};
    EXPECT_CALL(tracing_runtime_mock_,
                Trace(BindingType::kLoLa,
                      trace_context_id,
                      expected_service_element_instance_identifier_view,
