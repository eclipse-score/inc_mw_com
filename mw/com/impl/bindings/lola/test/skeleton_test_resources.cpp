...
const bmw::os::Fcntl::Open kCreateOrOpenFlags{bmw::os::Fcntl::Open::kCreate | bmw::os::Fcntl::Open::kReadOnly};
    ...
    amp::optional<std::size_t> size)
{
    ...
    {
        lola_service_instance_deployment_.events_[lola_event_inst_depl.first] = lola_event_inst_depl.second;
    }

    for (auto lola_field_inst_depl : lola_field_inst_depls)
    {
        lola_service_instance_deployment_.fields_[lola_field_inst_depl.first] = lola_field_inst_depl.second;
    }
...
SkeletonMockedMemoryFixture::SkeletonMockedMemoryFixture()
{
    ...
    EXPECT_CALL(
        *(std::static_pointer_cast<::testing::StrictMock<memory::shared::SharedMemoryResourceHeapAllocatorMock>>(
...
void SkeletonMockedMemoryFixture::InitialiseSkeleton(const InstanceIdentifier& instance_identifier)
{
    ...
    ON_CALL(filesystem_fake_.GetUtils(), CreateDirectories(_, _)).WillByDefault(Return(bmw::ResultBlank{}));
    ...
    const std::int32_t existence_marker_file_descriptor) noexcept
{
    ...
    EXPECT_CALL(*fcntl_mock_, flock(existence_marker_file_descriptor, unlock_operation)).WillOnce(Return(amp::blank{}));
}
