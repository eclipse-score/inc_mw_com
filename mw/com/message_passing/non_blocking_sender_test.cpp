...
class NonBlockingSenderFixture : public ::testing::Test
{
  public:
    ...
    {
        auto sender_mock = amp::pmr::make_unique<SenderMock>(amp::pmr::get_default_resource());
    ...
    amp::optional<NonBlockingSender> unit_{};
    ...
    amp::pmr::unique_ptr<concurrency::Task> current_task_{};
};
...
TEST_P(NonBlockingSenderParameterized, SendMessage_EmptyQueue)
{
    ...
    EXPECT_CALL(executorMock_, Enqueue(_)).WillOnce(Invoke([this](amp::pmr::unique_ptr<concurrency::Task> task) {
        current_task_ = std::move(task);
    }));

    // when we send a message on the unit.
    amp::expected_blank<bmw::os::Error> result;
    ...
    if (messageSizeType == MessageSizeType::SHORT_MESSAGE)
    {
        EXPECT_CALL(*sender_mock_raw_ptr_, Send(An<const message_passing::ShortMessage&>()))
            ...
            .WillOnce(Return(wrappedSenderResult == WrappedSenderResult::SEND_OK
                                 ? amp::expected_blank<bmw::os::Error>()
                                 : amp::make_unexpected(bmw::os::Error::createFromErrno(EBUSY))));
    }
    else
    {
...
TEST_F(NonBlockingSenderFixture, SendShortMessage_FullQueue)
{
    ...
    EXPECT_FALSE(result);
    EXPECT_EQ(result.error(), bmw::os::Error::createFromErrno(EAGAIN));
}
...
TEST_F(NonBlockingSenderFixture, SendShortMessage_MultipleFromQueue)
{
    ...
    EXPECT_CALL(executorMock_, Enqueue(_))
        .Times(1)
        .WillOnce(Invoke([this](amp::pmr::unique_ptr<concurrency::Task> task) { current_task_ = std::move(task); }));
    ...
    amp::stop_source stop_source;
    EXPECT_CALL(*sender_mock_raw_ptr_, Send(An<const ShortMessage&>())).Times(2).WillRepeatedly(Return(amp::blank{}));
