...
#include "platform/aas/mw/com/impl/skeleton_field.h"
...
class MyDummyField final : public SkeletonField<TestSampleType>
{
  public:
    MyDummyField(SkeletonBase& skeleton_base, const amp::string_view field_name)
        : SkeletonField<TestSampleType>{skeleton_base, field_name}
    {
    }
};
...
{
  public:
    MyDummyEvent(SkeletonBase& skeleton_base, const amp::string_view event_name)
...
class MyDummySkeleton final : public SkeletonBase
{
  public:
    ...
    MyDummyEvent dummy_event2{*this, kDummyEventName2};

    MyDummyField dummy_field{*this, kDummyFieldName};
};
  ...
  public:
    ...
    void ExpectEventCreation(const InstanceIdentifier& instance_identifier) noexcept
    {
        ...
        EXPECT_CALL(skeleton_event_binding_factory_mock_guard_.factory_mock_,
                    Create(instance_identifier, _, amp::string_view{kDummyEventName}))
            .WillOnce(Return(ByMove(std::move(skeleton_event_mock_ptr_1))));
        EXPECT_CALL(skeleton_event_binding_factory_mock_guard_.factory_mock_,
                    Create(instance_identifier, _, amp::string_view{kDummyEventName2}))
        ...
        EXPECT_CALL(skeleton_field_binding_factory_mock_guard_.factory_mock_,
                    CreateEventBinding(instance_identifier, _, amp::string_view{kDummyFieldName}))
    ...
    void ExpectOfferService() noexcept
    {
        ...
        EXPECT_CALL(*binding_mock_, FinalizeOffer());
    ...
    const ServiceTypeDeployment type_deployment_{amp::blank{}};
...
TEST_F(SkeletonBaseOfferFixture, TerminateOnOfferWithNoBinding)
{
    ...
    auto offer_unit_with_prepare_offering_failure = [this]() {
        ...
        EXPECT_DEATH({ amp::ignore = skeleton_->OfferService(); }, ".*");
    };
...
TEST_F(SkeletonBaseMoveFixture, MovingConstructingSkeletonBaseDoesNotCallPrepareStopOffer)
{
    ...
    std::size_t event_stop_offer_called_count{0};
    {
        ...
        EXPECT_CALL(*event_binding_mock_2_, PrepareOffer());
        EXPECT_CALL(*field_binding_mock_, PrepareOffer());
        EXPECT_CALL(*binding_mock, FinalizeOffer());
        EXPECT_CALL(service_discovery_mock_, OfferService(_));

        // and Expecting that PrepareOffer gets called on the skeleton binding and each event / field
        EXPECT_CALL(service_discovery_mock_, StopOfferService(_));
        EXPECT_CALL(*event_binding_mock_1_, PrepareStopOffer())
            .WillRepeatedly(Invoke([&event_stop_offer_called_count]() { event_stop_offer_called_count++; }));
        EXPECT_CALL(*event_binding_mock_2_, PrepareStopOffer())
            .WillRepeatedly(Invoke([&event_stop_offer_called_count]() { event_stop_offer_called_count++; }));
...
TEST_F(SkeletonBaseMoveFixture, MovingAssigningOfferedSkeletonBaseCallsPrepareStopOffer)
{
    ...
    EXPECT_CALL(skeleton_event_binding_factory_mock_guard_.factory_mock_,
                Create(valid_instance_identifier2, _, amp::string_view{kDummyEventName}))
        .WillOnce(Return(ByMove(std::move(skeleton_event_mock_ptr_1))));
    EXPECT_CALL(skeleton_event_binding_factory_mock_guard_.factory_mock_,
                Create(valid_instance_identifier2, _, amp::string_view{kDummyEventName2}))
        .WillOnce(Return(ByMove(std::move(skeleton_event_mock_ptr_2))));
    EXPECT_CALL(skeleton_field_binding_factory_mock_guard_.factory_mock_,
                CreateEventBinding(valid_instance_identifier2, _, amp::string_view{kDummyFieldName}))
        .WillOnce(Return(ByMove(std::move(skeleton_field_mock_ptr))));
    ...
    EXPECT_CALL(*second_binding_mock, FinalizeOffer());
...
TEST_F(SkeletonBaseMoveFixture, MovingAssigningUnOfferedSkeletonBaseDoesNotCallPrepareStopOffer)
{
    ...
    EXPECT_CALL(*binding_mock, FinalizeOffer()).Times(0);
...
TEST_F(SkeletonBaseOfferFixture, ServiceCanBeReOfferedAfterMoveConstructingService)
{
    ...
    EXPECT_CALL(*binding_mock, FinalizeOffer()).Times(2);
...
TEST_F(SkeletonBaseOfferFixture, ServiceCanBeReOfferedAfterCallingStopOfferService)
{
...

    {
        ...
        EXPECT_CALL(*binding_mock, PrepareOffer(_, _, _))
            ...
            .WillRepeatedly(Invoke(
                [&skeleton_offer_count](SkeletonBinding::SkeletonEventBindings&,
                                        ...
                                        amp::optional<SkeletonBinding::RegisterShmObjectTraceCallback>) -> ResultBlank {
        ...
        EXPECT_CALL(*event_binding_mock_1_, PrepareStopOffer())
            .Times(2)
            .WillRepeatedly(Invoke([&event_stop_offer_count]() { event_stop_offer_count++; }));
        EXPECT_CALL(*event_binding_mock_2_, PrepareStopOffer())
            .Times(2)
            .WillRepeatedly(Invoke([&event_stop_offer_count]() { event_stop_offer_count++; }));
        EXPECT_CALL(*field_binding_mock_, PrepareStopOffer())
        ...
        EXPECT_CALL(*binding_mock, PrepareStopOffer(_))
            .Times(2)
            .WillRepeatedly(Invoke([&skeleton_stop_offer_count]() -> void { skeleton_stop_offer_count++; }));
...
TEST_F(SkeletonBaseOfferFixture, NoStopOfferOnErrorIdentifier)
{
    ...
    EXPECT_CALL(skeleton_event_binding_factory_mock_guard_.factory_mock_,
        ...
        .Times(0);
    EXPECT_CALL(skeleton_field_binding_factory_mock_guard_.factory_mock_,
                CreateEventBinding(GetInstanceIdentifierWithoutBinding(), _, amp::string_view{kDummyFieldName}))
        .Times(0);
