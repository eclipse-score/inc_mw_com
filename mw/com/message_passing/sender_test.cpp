...
#include "platform/aas/mw/com/message_passing/sender_traits_mock.h"
...
const amp::string_view kSomeValidPath = "foo";
...
/// \attention As far as \c amp::callback has passing memory limitations the static members are used here
...
{
  protected:
    void SetUp() override { ForwardingSenderChannelTraits::impl_ = &mock_; }
    void TearDown() override { ForwardingSenderChannelTraits::impl_ = nullptr; }

    /// \brief Instructs channel mock to succeed on any "try_open" and expect any number of (final) "close_sender"
    ///        calls.
    void PrepareChannel(const amp::string_view identifier = kSomeValidPath)
    {
        ...
        EXPECT_CALL(mock_, try_open(kSomeValidPath, _))
            .WillRepeatedly(
                Invoke([counter, this](auto, auto) mutable
                       -> amp::expected<ForwardingSenderChannelTraits::file_descriptor_type, bmw::os::Error> {
                    if (counter < kRequestStopAfterRetries)
                    {
                        ++counter;
                        lifecycle_source_.request_stop();
                    }
                    return amp::make_unexpected(bmw::os::Error::createFromErrno(ENOENT));
                }));
    }

    SenderChannelTraitsMock mock_{};
    amp::stop_source lifecycle_source_{};
...
TEST_F(SenderFixture, WillCreateUnitIfMessageChannelIsFound)
{
    ...
    amp::ignore = SenderFactoryImplMock::Create(kSomeValidPath, lifecycle_source_.get_token(), {}, log_clb);
...
TEST_F(SenderFixture, WillWaitUntilChannelIsFound)
{
    ...
    EXPECT_CALL(mock_, try_open(kSomeValidPath, _))
        .WillRepeatedly(Invoke(
            [&counter](auto,
                       auto) -> amp::expected<ForwardingSenderChannelTraits::file_descriptor_type, bmw::os::Error> {
...
TEST_F(SenderFixture, SendMessageOnFirstTry)
{
    ...
    EXPECT_CALL(mock_, try_send(_, _, _)).WillOnce(Return(amp::expected_blank<bmw::os::Error>{}));
...
TEST_F(SenderFixture, RetryToSendMessage)
{
    ...
    EXPECT_CALL(mock_, try_send(_, _, _))
        .WillRepeatedly(Invoke([&invoke_counter](auto, auto, auto) -> amp::expected_blank<bmw::os::Error> {
            if (invoke_counter < kSendSuccesfulAfterRetry)
            {
                ++invoke_counter;
                return amp::make_unexpected(bmw::os::Error::createFromErrno(EOF));
            }
...
TEST_F(SenderFixture, StopRetryAfter5Times)
{
    ...
    EXPECT_EQ(result.error(), bmw::os::Error::createFromErrno(EOF));
...
TEST_F(SenderFixture, StopAfterStopRequest)
{
    ...
    EXPECT_CALL(mock_, try_send(_, _, _))
        .WillRepeatedly(Invoke([&invoke_counter, this](auto, auto, auto) -> amp::expected_blank<bmw::os::Error> {
            ++invoke_counter;
...
TEST_F(SenderFixture, SenderShouldRetrySendingWithDelay)
{
    ...
    EXPECT_CALL(mock_, try_send(_, _, _))
        .WillRepeatedly(Invoke([](auto, auto, auto) -> amp::expected_blank<bmw::os::Error> {
            return amp::make_unexpected(bmw::os::Error::createFromErrno(EOF));
        }));
    EXPECT_CALL(mock_, prepare_payload(::testing::An<const ShortMessage&>()));
