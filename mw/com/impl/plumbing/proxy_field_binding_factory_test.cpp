...
using ProxyFieldBindingFactoryFixture = lola::ProxyMockedMemoryFixture;
TEST_F(ProxyFieldBindingFactoryFixture, CreateLolaProxy)
{
    ...
    LolaServiceInstanceDeployment shm_binding_information{LolaServiceInstanceId{instance_id}};
    shm_binding_information.fields_[kFieldName] = LolaFieldInstanceDeployment{};
    ...
    lola_service_type_deployment.fields_[kFieldName] = 5U;
    ...
    std::unique_ptr<ProxyEventBinding<std::uint8_t>> proxy_event =
        ProxyFieldBindingFactory<std::uint8_t>::CreateEventBinding(proxy_base, kFieldName);
    ...
    proxy_event->GetNewSamples(
        [](SamplePtr<std::uint8_t> sample, tracing::ITracingRuntime::TracePointDataId) { EXPECT_EQ(*sample, 42); },
        guard_factory);
}
