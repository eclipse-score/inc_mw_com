...
using TestFieldType = std::uint16_t;

const ServiceTypeDeployment kEmptyTypeDeployment{amp::blank{}};
...
class DummyProxy : public ProxyBase
{
  public:
    ...
    bool AreBindingsValid() const noexcept { return ProxyBase::AreBindingsValid(); }
};

DummyProxy kEmptyProxy(std::make_unique<mock_binding::Proxy>(),
...
class ProxyEventBaseFixture : public ::testing::Test
{
  public:
    ...
    void CreateServiceElement()
    {
        ...
        service_element_ =
            std::make_unique<ServiceElementType>(kEmptyProxy, std::move(mock_service_element_binding_ptr), kEventName);

        ON_CALL(*mock_service_element_binding_, SetReceiveHandler(_)).WillByDefault(Return(amp::blank{}));
    }
...
{
    using ServiceElementType = ProxyEventBase;
    using MockServiceElementType = mock_binding::ProxyEventBase;
};
...
{
    using ServiceElementType = ProxyField<TestFieldType>;
    using MockServiceElementType = mock_binding::ProxyEvent<TestFieldType>;
};
           ...
           CreatingServiceElementWithValidProxyEventBindingWillRegisterAndUnregisterEventBinding)
{
    ...
    EXPECT_CALL(*mock_proxy_binding,
                RegisterEventBinding(amp::string_view{kEventName}, Ref(*mock_proxy_event_binding)));
    EXPECT_CALL(*mock_proxy_binding, UnregisterEventBinding(amp::string_view{kEventName}));
...
{
    this->RecordProperty("Verifies", "7, 9, 1, 8");
...
class AServiceElement : public ::testing::Test
{
  public:
    ...
    AServiceElement() { CreateAServiceElement(); }
    ...
    AServiceElement& ThatCapturesTheBindingDependentHandler()
    {
        // and given that the receive handler is set by the binding
        ON_CALL(*this->mock_service_element_binding_, SetReceiveHandler(_))
            .WillByDefault(Invoke([this](BindingEventReceiveHandler handler) -> ResultBlank {
    ...
    BindingEventReceiveHandler GetBindingDependentHandler()
    {
    ...
    // Used to save the BindingEventReceiveHandler passed to the binding dependent SetReceiveHandler. Since gtest
    // currently seems to require that the lambdas provided to Invoke are copyable, we pass it in using a shared_ptr
    std::shared_ptr<std::promise<BindingEventReceiveHandler>> event_receive_handler_promise_{
        std::make_shared<std::promise<BindingEventReceiveHandler>>()};
...
{
    this->RecordProperty("Verifies", "6, 4, 4");
    ...
    EXPECT_CALL(*this->mock_service_element_binding_, SetReceiveHandler(_)).WillOnce(Return(amp::blank{}));

    // When setting a receive-handler
    amp::ignore = this->service_element_->SetReceiveHandler(EventReceiveHandler{});
}
...
{
    this->RecordProperty("Verifies", "2, 5, 5");
    ...
    EXPECT_CALL(*this->mock_service_element_binding_, UnsetReceiveHandler()).WillOnce(Return(amp::blank{}));

    // and un-setting the receive-handler
    amp::ignore = this->service_element_->UnsetReceiveHandler();
}
...
TYPED_TEST(AServiceElement, WhenUnsettingAReceiveHandlerWithoutSettingThenItWillDispatchToBinding)
{
    // Given a Service Element, that is connected to a mock binding
...
TYPED_TEST(AServiceElement, WhenSettingAReceiveHandlerThenTheLifetimeOfTheBindingEventReceiveHandlerIsValid)
{
    this->RecordProperty("Verifies", "6");
    this->RecordProperty(
        "Description",
        "Checks that the lifetime of the BindingEventReceiveHandler is active when SetReceiveHandler is called.");
    this->RecordProperty("TestType", "Requirements-based test");
    this->RecordProperty("Priority", "1");
    this->RecordProperty("DerivationTechnique", "Analysis of requirements");

    // Given a Service Element, that is connected to a mock binding that captures the binding dependent handler
    this->ThatCapturesTheBindingDependentHandler();

    // When setting the receive handler
    amp::ignore = this->service_element_->SetReceiveHandler(EventReceiveHandler{[]() {}});

    // Then the lifetime of the scoped function should be valid
    auto binding_event_receive_handler = this->GetBindingDependentHandler();
    EXPECT_TRUE(binding_event_receive_handler().has_value());
}

TYPED_TEST(AServiceElement,
           WhenUnSettingAReceiveHandlerAfterSettingThenTheLifetimeOfTheBindingEventReceiveHandlerIsInvalid)
{
    this->RecordProperty("Verifies", "6");
    this->RecordProperty(
        "Description",
        "Checks that the lifetime of the BindingEventReceiveHandler is ended when UnsetReceiveHandler is called.");
    this->RecordProperty("TestType", "Requirements-based test");
    this->RecordProperty("Priority", "1");
    this->RecordProperty("DerivationTechnique", "Analysis of requirements");
    ...
    // dependent handler
    this->ThatCapturesTheBindingDependentHandler().WithAReceiveHandler();

    // When un-setting the receive-handler
    this->service_element_->UnsetReceiveHandler();

    // Then the lifetime of the scoped function should be invalid
    auto binding_event_receive_handler = this->GetBindingDependentHandler();
    EXPECT_FALSE(binding_event_receive_handler().has_value());
}

TYPED_TEST(ProxyEventBaseSubscribeFixture, CallingSubscribeWhileUnsubscribedDispatchesToBinding)
{
    this->RecordProperty("Verifies", "8, 8, 0");
...
TYPED_TEST(ProxyEventBaseSubscribeFixture, CallingSubscribeTwiceWithSameMaxSamplesOnlyPerformsSubscriptionOnce)
{
    ...
    EXPECT_CALL(*this->mock_service_element_binding_, GetSubscriptionState())
        .WillOnce(Return(SubscriptionState::kNotSubscribed));
    EXPECT_CALL(*this->mock_service_element_binding_, Subscribe(7)).WillOnce(Return(amp::blank{}));
    EXPECT_CALL(*this->mock_service_element_binding_, GetSubscriptionState())
        .WillOnce(Return(SubscriptionState::kSubscriptionPending));
...
{
    this->RecordProperty("Verifies", "5, 0, 2");
    this->RecordProperty("Description",
...
{
    this->RecordProperty("Verifies", "1, 2, 6, 1");
    this->RecordProperty(
        "Description",
    ...
    amp::optional<TrackerGuardFactory> tracker_guard_factory{tracker.Allocate(2)};
...
{
    this->RecordProperty("Verifies", "2, 3, 7");
    this->RecordProperty(
        "Description",
...
TEST(ProxyEventBaseTest, MoveConstructingProxyEventDoesNotCrash)
{
    ...
    // Given a Service Element, that is connected to a mock binding
    ProxyEventBase dummy_event{kEmptyProxy, std::move(mock_proxy_event_binding_ptr), kEventName};
    ...
    EXPECT_CALL(mock_proxy_event_binding, Subscribe(7)).WillOnce(Return(amp::blank{}));
...
TEST(ProxyEventBaseTest, MoveAssigningProxyEventDoesNotCrash)
{
    ...
    ProxyEventBase dummy_event_2{kEmptyProxy, std::make_unique<StrictMock<mock_binding::ProxyEventBase>>(), kEventName};
