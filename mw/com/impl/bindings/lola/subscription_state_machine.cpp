...
SubscriptionStateBase& SubscriptionStateMachine::GetCurrentEventState() const noexcept
{
    // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-constant-array-index): current_state_idx_ can't be const expression
    return *states_[static_cast<std::uint8_t>(current_state_idx_)];
}
...
void SubscriptionStateMachine::SetReceiveHandler(BindingEventReceiveHandler handler) noexcept
{
    std::lock_guard<std::mutex> lock{state_mutex_};
...
amp::optional<std::uint16_t> SubscriptionStateMachine::GetMaxSampleCount() const noexcept
{
    std::lock_guard<std::mutex> lock{state_mutex_};
    return GetCurrentEventState().GetMaxSampleCount();
}

amp::optional<SlotCollector>& SubscriptionStateMachine::GetSlotCollectorLockFree() noexcept
{
    return GetCurrentEventState().GetSlotCollector();
}

const amp::optional<SlotCollector>& SubscriptionStateMachine::GetSlotCollectorLockFree() const noexcept
{
    return GetCurrentEventState().GetSlotCollector();
}

amp::optional<TransactionLogSet::TransactionLogIndex> SubscriptionStateMachine::GetTransactionLogIndex() const noexcept
{
    std::lock_guard<std::mutex> lock{state_mutex_};
