...
#include "platform/aas/mw/com/impl/bindings/lola/event_data_control_test_resources.h"
...
void EventDataControlCompositeAttorney::PrepareAllocateNextSlot(
    const std::pair<amp::optional<EventDataControl::SlotIndexType>, bool> expected_result) noexcept
{
    event_data_control_composite_.ignore_qm_control_ = expected_result.second;
    const auto slot_index = expected_result.first.has_value()
                                ? expected_result.first.value()
                                : std::numeric_limits<EventDataControl::SlotIndexType>::max();
...
std::pair<EventSlotStatus, amp::optional<EventSlotStatus>> EventDataControlCompositeAttorney::GetSlotStatus(
    const EventDataControl::SlotIndexType slot_index) const noexcept
{
    return {
        ...
        event_data_control_composite_.asil_b_control_ != nullptr
            ? amp::optional<EventSlotStatus>(event_data_control_composite_.asil_b_control_->state_slots_[slot_index])
            : amp::optional<EventSlotStatus>{}};
}
...
void EventDataControlAttorney::PrepareAllocateNextSlot(
    const amp::optional<EventDataControl::SlotIndexType> expected_result) noexcept
{
    const auto slot_index = expected_result.has_value() ? expected_result.value()
                                                        : std::numeric_limits<EventDataControl::SlotIndexType>::max();
...
void EventDataControlAttorney::PrepareReferenceNextEvent(
    const amp::optional<EventDataControl::SlotIndexType> expected_result,
    const EventSlotStatus::EventTimeStamp last_search_time,
    const EventSlotStatus::EventTimeStamp upper_limit) noexcept
{
    AMP_ASSERT_MESSAGE(upper_limit > last_search_time, "upper_limit needs to be larger than last_search_time");
    const auto number_state_slots = event_data_control_.state_slots_.size();
...
{
    AMP_ASSERT_MESSAGE(expected_result <= event_data_control_.state_slots_.size(),
