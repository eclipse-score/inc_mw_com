...
///     - platform/aas/mw/com/impl/proxy_event_base_test.cpp contains unit tests which contain paramaterised tests to
...
///     - platform/aas/mw/com/impl/proxy_event_test.cpp contains unit tests which contain paramaterised tests to re-use
...
TEST(GenericProxyEventTest, DieOnProxyDestructionWhileHoldingSamplePtrs)
{
    ...
    amp::optional<SamplePtr<SampleType>> ptr{};
    proxy_event->Subscribe(max_num_samples);
    Result<std::size_t> num_samples =
        proxy_event->GetNewSamples([&ptr](SamplePtr<SampleType> new_sample) { ptr = std::move(new_sample); }, 1U);
...
TEST(GenericProxyEventGetNewSamplesTest, GetNewSamplesContainsCorrectReceiverSignature)
{
    ...
    amp::ignore = proxy_event.GetNewSamples(std::move(get_new_samples_callable), max_num_samples);
