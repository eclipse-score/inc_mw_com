...
TEST(SkeletonEventTracingTest, TracePointsAreDisabledIfConfigNotReturnedByRuntime)
{
    ...
    EXPECT_CALL(skeleton_event_binding_factory_mock_guard.factory_mock_,
                Create(kInstanceIdentifier, _, amp::string_view{kEventName}))
...
TEST_P(SkeletonEventTracingParamaterisedFixture, TracePointsAreCorrectlySet)
{
    ...
    EXPECT_CALL(tracing_mock,
                IsTracePointEnabled(service_type, amp::string_view{kEventName}, _, SkeletonEventTracePointType::SEND))
        .WillOnce(Return(expected_enabled_trace_points.enable_send));
    EXPECT_CALL(tracing_mock,
                IsTracePointEnabled(
                    service_type, amp::string_view{kEventName}, _, SkeletonEventTracePointType::SEND_WITH_ALLOCATE))
...
class SkeletonEventTracingFixture : public ::testing::Test
{
  public:
        ...
        SkeletonEventBindingFactoryMockGuard<TestSampleType>& factory_mock_guard)
    {
        ...
        EXPECT_CALL(factory_mock_guard.factory_mock_, Create(kInstanceIdentifier, _, amp::string_view{kEventName}))
            .WillOnce(Return(ByMove(std::move(skeleton_event_binding_mock_ptr))));
    }
...
{
    RecordProperty("Verifies", "8, 5, 1, 6, 7");
    RecordProperty("Description",
                   ...
                   "(5, 1, 6). The Send trace points are traced with a "
                   "TracePointDataId (7).");
