...
// When  is implemented, the Update call does not have to be deferred until OfferService is called. This test
...
TEST(SkeletonFieldCopyUpdateTest, CallingUpdateBeforeOfferServiceDefersCallToOfferService)
{
    ...
    EXPECT_CALL(skeleton_field_binding_factory_mock_guard.factory_mock_,
                CreateEventBinding(kInstanceIdWithLolaBinding, _, amp::string_view{kFieldName}))
...
// This test can be removed when  is implemented.
...
/// \todo When  is implemented, Allocate() should propagate an error code rather than returning a nullptr.
...
TEST(SkeletonFieldInitialValueFixture, MoveAssigningFieldBeforePrepareOfferWillKeepInitialValue)
{
    ...
    EXPECT_CALL(skeleton_field_binding_factory_mock_guard.factory_mock_,
                CreateEventBinding(identifier2, _, amp::string_view{kFieldName}))
