  ...
  protected:
    StateMachineMethodsFixture()
        : LolaProxyEventResources(),
          state_machine_{parent_->GetQualityType(),
                         element_fq_id_,
                         kDummyPid,
                         parent_->GetEventControl(element_fq_id_),
    ...
    BindingEventReceiveHandler CreateMockBindingEventReceiveHandler(StrictMock<MockFunction<void()>>& mock_function)
    {
...
TEST_F(StateMachineMethodsNotSubscribedStateFixture, CallingSubscribeWillRegisterLatestReceiveHandler)
{
    ...
    state_machine_.SetReceiveHandler(CreateMockBindingEventReceiveHandler(first_receive_handler));
    state_machine_.SetReceiveHandler(CreateMockBindingEventReceiveHandler(second_receive_handler));

    amp::ignore = state_machine_.SubscribeEvent(max_num_slots_);
       ...
       CallingSubscribeAfterReofferWillRegisterReceiveHandlerWithLatestPid)
{
    ...
    state_machine_.SetReceiveHandler(CreateMockBindingEventReceiveHandler(receive_handler));
...
TEST_F(StateMachineMethodsSubscriptionPendingStateFixture, CallingReofferWillRegisterNewestReceiveHandler)
{
    ...
    // and that the receive handler registered while in NotSubscribed will be registered with the first PID
    amp::ignore = ExpectRegisterEventNotification();
    ...
    state_machine_.SetReceiveHandler(CreateMockBindingEventReceiveHandler(receive_handler_not_subscribed));

    // When we transition to the subscribed state and then to subscription pending
    EnterSubscriptionPending(max_num_slots_);

    state_machine_.SetReceiveHandler(CreateMockBindingEventReceiveHandler(receive_handler_subscription_pending));
