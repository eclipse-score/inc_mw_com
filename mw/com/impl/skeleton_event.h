...
#ifndef PLATFORM_AAS_MW_COM_IMPL_SKELETON_EVENT_H
#define PLATFORM_AAS_MW_COM_IMPL_SKELETON_EVENT_H
...
class SkeletonEvent : public SkeletonEventBase
{
  ...
  public:
    ...
    SkeletonEvent(SkeletonBase& skeleton_base, const amp::string_view event_name);
...
template <typename SampleDataType>
SkeletonEvent<SampleDataType>::SkeletonEvent(SkeletonBase& skeleton_base, const amp::string_view event_name)
...
template <typename SampleDataType>
SkeletonEvent<SampleDataType>& SkeletonEvent<SampleDataType>::operator=(SkeletonEvent&& other) & noexcept
{
    if (this != &other)
    {
...
ResultBlank SkeletonEvent<SampleDataType>::Send(const EventType& sample_value) noexcept
{
    ...
    {
        bmw::mw::log::LogError("lola") << "SkeletonEvent::Send with copy failed as Event has not yet been offered";
    ...
    {
        bmw::mw::log::LogError("lola") << "SkeletonEvent::Send with copy failed: " << send_result.error().Message()
...
ResultBlank SkeletonEvent<SampleDataType>::Send(SampleAllocateePtr<EventType> sample) noexcept
{
    ...
    if (!send_result.has_value())
    {
        bmw::mw::log::LogError("lola") << "SkeletonEvent::Send zero copy failed: " << send_result.error().Message()
                                       << ": " << send_result.error().UserMessage();
        return MakeUnexpected(ComErrc::kBindingFailure);
    }
    return send_result;
...
Result<SampleAllocateePtr<SampleDataType>> SkeletonEvent<SampleDataType>::Allocate() noexcept
{
    if (!service_offered_flag_.IsSet())
    {
        bmw::mw::log::LogError("lola") << "SkeletonEvent::Allocate failed as Event has not yet been offered";
        return MakeUnexpected(ComErrc::kNotOffered);
    }
    ...
    {
        bmw::mw::log::LogError("lola") << "SkeletonEvent::Allocate failed: " << allocate_result.error().Message()
...
template <typename SampleDataType>
SkeletonEventBinding<SampleDataType>* SkeletonEvent<SampleDataType>::GetTypedEventBinding() const noexcept
{
    auto* const typed_binding = dynamic_cast<SkeletonEventBinding<EventType>*>(binding_.get());
    AMP_ASSERT_PRD_MESSAGE(typed_binding != nullptr, "Downcast to SkeletonEventBinding<EventType> failed!");
...
class SkeletonEventView
{
  public:
    ...
    SkeletonEventBinding<SampleType>* GetBinding() const noexcept { return skeleton_event_.GetTypedEventBinding(); }
...
#endif  // PLATFORM_AAS_MW_COM_IMPL_SKELETON_EVENT_H
