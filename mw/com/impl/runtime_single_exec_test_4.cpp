...
/// \brief TC verifies, that a 2nd call to Runtime::Initialize() succeeds but configuration is not activated, because
///        Runtime::getInstance() has been called before 2nd Initialize() call!
TEST(RuntimeSingleProcessTest, initSecondTimeDoesNotUpdateRuntime)
{
    ...
    auto identifiers = Runtime::getInstance().resolve(instance_specifier_result_1.value());
    EXPECT_EQ(identifiers.size(), 1);
    Runtime::Initialize(test_args_span2);
    identifiers = Runtime::getInstance().resolve(instance_specifier_result_2.value());
    EXPECT_EQ(identifiers.size(), 0);
    identifiers = Runtime::getInstance().resolve(instance_specifier_result_1.value());
    EXPECT_EQ(identifiers.size(), 1);
}
