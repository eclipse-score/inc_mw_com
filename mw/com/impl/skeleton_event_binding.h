...
#ifndef PLATFORM_AAS_MW_COM_IMPL_SKELETON_EVENT_BINDING_H
#define PLATFORM_AAS_MW_COM_IMPL_SKELETON_EVENT_BINDING_H
...
{
  public:
    using SubscribeTraceCallback = amp::callback<void(std::size_t, bool), 64U>;
    using UnsubscribeTraceCallback = amp::callback<void(), 64U>;
...
{
  public:
    using SendTraceCallback = amp::callback<void(SampleAllocateePtr<SampleType>&), 64U>;

    /// \brief SampleType is allocated by the user and provided to the middleware to send
    /// \return On failure, returns an error code.
    virtual ResultBlank Send(const SampleType&, amp::optional<SendTraceCallback>) noexcept = 0;
    ...
    /// \return On failure, returns an error code.
    virtual ResultBlank Send(SampleAllocateePtr<SampleType>, amp::optional<SendTraceCallback>) noexcept = 0;
    ...
    std::size_t GetMaxSize() const noexcept override { return sizeof(SampleType); }
};
...
#endif  // PLATFORM_AAS_MW_COM_IMPL_SKELETON_EVENT_BINDING_H
