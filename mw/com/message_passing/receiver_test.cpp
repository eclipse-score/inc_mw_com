...
#include "platform/aas/mw/com/message_passing/receiver_traits_mock.h"
...
class ReceiverFixture : public ::testing::Test
{
  public:
    ...
    void SetUp() override
    {
        ...
        unit_ = ReceiverFactoryMock::Create(SOME_PATH, thread_pool_, amp::span<const uid_t>{}, receiver_config);
    }
    ...
    bmw::concurrency::ThreadPool thread_pool_{1};
    ReceiverChannelTraitsMock mock_{};
    amp::pmr::unique_ptr<bmw::mw::com::message_passing::IReceiver> unit_{};
};
...
TEST_F(ReceiverFixture, CanRegisterACallback)
{
    ...
    unit_->Register(0x42,
                    amp::callback<void(const ShortMessagePayload, const pid_t)>{
...
TEST_F(ReceiverFixture, CanNotOpenUnderlyingChannel)
{
    ...
    EXPECT_CALL(mock_, open_receiver(_, _, _, _))
        .WillOnce(Return(amp::make_unexpected(bmw::os::Error::createFromErrno(EOF))));
...
TEST_F(ReceiverFixture, DestructReceiverWhileListeningWillStop)
{
    ...
    auto unit = ReceiverFactoryMock::Create(SOME_PATH, thread_pool_, amp::span<const uid_t>{}, ReceiverConfig{});
...
TEST_F(ReceiverFixture, CorrectCallbackIsInvokedForProperMessage)
{
    ...
    unit_->Register(
        0x50,
        amp::callback<void(const MediumMessagePayload, const pid_t)>{
    ...
    unit_->Register(0x43,
                    amp::callback<void(const ShortMessagePayload, const pid_t)>{
                        [](const ShortMessagePayload, const pid_t) noexcept { ASSERT_FALSE(true); }});
    ...
    EXPECT_CALL(mock_, receive_next)
        .WillOnce(
            Invoke([](const file_descriptor_type /*file_descriptor*/,
                      ...
                      const FileDescriptorResourcesType& /*os_resources*/) -> amp::expected<bool, bmw::os::Error> {
        ...
        .WillOnce(
            Invoke([](const file_descriptor_type,
                      std::size_t,
                      ShortMessageProcessor,
                      MediumMessageProcessor,
                      const FileDescriptorResourcesType&) -> amp::expected<bool, bmw::os::Error> { return false; }));
