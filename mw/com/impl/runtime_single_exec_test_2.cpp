...
///        consecutive call to Runtime::resolve() does NOT return a bmw::mw::com::InstanceIdentifiers, when called with
///        an invalid (not configured) specifier.
///
/// \note we are re-using the existing //platform/aas/mw/com/impl/configuration:example/ara_com_config.json manifest
///       in this test.
TEST(RuntimeSingleProcessTest, initValidManifestPathDoesntReturnsWithInvalidInstanceSpecifier)
{
    ...
    const auto instance_specifier_result = InstanceSpecifier::Create("abc/abc/TirePressurePort_fail");
