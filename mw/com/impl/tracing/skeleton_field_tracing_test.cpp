...
TEST(SkeletonFieldTracingTest, TracePointsAreDisabledIfConfigNotReturnedByRuntime)
{
    ...
    EXPECT_CALL(skeleton_field_binding_factory_mock_guard.factory_mock_,
                CreateEventBinding(kInstanceIdentifier, _, amp::string_view{kFieldName}))
...
TEST_P(SkeletonFieldTracingParamaterisedFixture, TracePointsAreCorrectlySet)
{
    ...
    const tracing::ServiceElementIdentifierView service_element_identifier_view{
        service_type, kFieldName, tracing::ServiceElementType::FIELD};
    ...
    EXPECT_CALL(tracing_mock,
                IsTracePointEnabled(service_type, amp::string_view{kFieldName}, _, SkeletonFieldTracePointType::UPDATE))
        .WillOnce(Return(expected_enabled_trace_points.enable_send));
    EXPECT_CALL(tracing_mock,
                IsTracePointEnabled(
                    service_type, amp::string_view{kFieldName}, _, SkeletonFieldTracePointType::UPDATE_WITH_ALLOCATE))
...
class SkeletonFieldTracingFixture : public ::testing::Test
{
  public:
    ...
    {
        const tracing::ServiceElementIdentifierView service_element_identifier_view{
            kServiceTypeName, kFieldName, tracing::ServiceElementType::FIELD};
        const tracing::ServiceElementInstanceIdentifierView expected_service_element_instance_identifier_view{
            service_element_identifier_view, amp::string_view{kInstanceSpecifier.ToString()}};
...
{
    RecordProperty("Verifies", "8, 5, 1, 7");
    RecordProperty("Description",
                   "The Trace point types for binding SkeletonField Send are correctly mapped "
                   "(8). The Send trace points are traced with a ShmDataChunkList "
                   "(5, 1). The Send trace points are traced with a TracePointDataId.");
    ...
    amp::optional<SkeletonEventBinding<TestSampleType>::SendTraceCallback> send_trace_callback_result{};
    EXPECT_CALL(*mock_skeleton_field_binding_, Send(sample_data, _))
        .WillOnce(WithArgs<1>(
            Invoke([&send_trace_callback_result](amp::optional<SkeletonEventBinding<TestSampleType>::SendTraceCallback>
