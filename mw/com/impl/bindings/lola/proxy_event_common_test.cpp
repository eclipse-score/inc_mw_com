...
BindingEventReceiveHandler FromMockFunction(safecpp::Scope<>& event_receive_handler_scope,
                                            ::testing::MockFunction<R(Args...)>& mock_function)
{
    return BindingEventReceiveHandler(event_receive_handler_scope, [&mock_function](Args&&... args) {
...
class LolaProxyEventCommonFixture : public ProxyMockedMemoryFixture
{
  public:
    ...
    void InitialiseProxyAndEvent() noexcept
    {
        ...
        proxy_event_ = std::make_unique<T>(*parent_, kElementFqId, kEventName);
    }
    ...
    void ExpectCallbackRegistration()
    {
        ...
        EXPECT_CALL(*mock_service_, RegisterEventNotification(QualityType::kASIL_QM, kElementFqId, _, kDummyPid))
            .WillOnce(::testing::Invoke([&](auto, auto, BindingEventReceiveHandler handler, auto) {
                handler();
...
{
    this->RecordProperty("Verifies", "4, 0, 8, 8");
...
{
    this->RecordProperty("Verifies", "4");
    this->RecordProperty(
        "Description",
    ...
    EXPECT_CALL(*this->mock_service_,
        ...
        .WillOnce(InvokeWithoutArgs([&handler_unregistered]() { handler_unregistered = true; }));
...
{
    this->RecordProperty("Verifies", "1, 3");
    this->RecordProperty(
        "Description",
