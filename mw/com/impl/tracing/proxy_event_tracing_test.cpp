...
#include "platform/aas/mw/com/impl/tracing/configuration/tracing_filter_config_mock.h"
...
class RuntimeMockGuard
{
  public:
    ...
    StrictMock<RuntimeMock> runtime_mock_;
};
...
class ProxyEventTracingFixture : public ::testing::Test
{
  public:
    ...
    void SetUp() override
    {
        ...
        ON_CALL(*mock_proxy_event_binding_, SetReceiveHandler(_)).WillByDefault(Return(amp::blank{}));
    }
    ...
    void ExpectProxyServiceElementBindingCreation(ProxyEventBindingFactoryMockGuard<TestSampleType>& factory_mock_guard)
    {
        ...
        EXPECT_CALL(factory_mock_guard.factory_mock_, Create(_, amp::string_view{kServiceElementName}))
    ...
    void ExpectProxyServiceElementBindingCreation(ProxyFieldBindingFactoryMockGuard<TestSampleType>& factory_mock_guard)
    {
        auto proxy_event_binding_mock_ptr = std::make_unique<mock_binding::ProxyEvent<TestSampleType>>();
        mock_proxy_event_binding_ = proxy_event_binding_mock_ptr.get();
        EXPECT_CALL(factory_mock_guard.factory_mock_, CreateEventBinding(_, amp::string_view{kServiceElementName}))
            .WillOnce(Return(ByMove(std::move(proxy_event_binding_mock_ptr))));
    }
        ...
        tracing::ProxyEventTracePointType) const noexcept
    {
        ...
        const tracing::ServiceElementInstanceIdentifierView expected_service_element_instance_identifier_view{
            service_element_identifier_view, amp::string_view{kInstanceSpecifier.ToString()}};
    ...
    void ExpectIsTracePointEnabledCalls(const tracing::ProxyEventTracingData& expected_enabled_trace_points,
                                        const amp::string_view service_type,
                                        const amp::string_view event_name,
                                        const amp::string_view instance_specifier_view) const noexcept
    {
...
TEST_P(ProxyEventTracingEnabledTracePointsParamaterizedFixture, TracePointsAreCorrectlySet)
{
    ...
    ExpectIsTracePointEnabledCalls<tracing::ProxyEventTracePointType>(expected_enabled_trace_points,
                                                                      ...
                                                                      amp::string_view{kServiceElementName},
                                                                      amp::string_view{kInstanceSpecifier.ToString()});
...
{
    this->RecordProperty("Verifies", "8, 1, 6");
    this->RecordProperty(
        "Description",
        "The Trace point types for ProxyEvent/ProxyField Subscibe are correctly mapped (8). The Subscribe "
        "trace points are traced with a LocalDataChunkList (1, 6).");
    ...
    EXPECT_CALL(tracing_runtime_mock,
                Trace(BindingType::kLoLa,
                      expected_service_element_instance_identifier_view,
                      trace_point_type,
                      amp::optional<tracing::ITracingRuntime::TracePointDataId>{},
                      _,
                      _))
...
{
    this->RecordProperty("Verifies", "9");
...
{
    this->RecordProperty("Verifies", "8");
    this->RecordProperty("Description",
...
{
    this->RecordProperty("Verifies", "8, 1, 5");
    this->RecordProperty("Description",
                         "The Trace point types for ProxyEvent/ProxyField Unsubscribe are correctly mapped "
                         "(8). The Unsubscribe "
                         "trace points are traced without a LocalDataChunkList (1, 5).");
    this->RecordProperty("TestType", "Requirements-based test");
    this->RecordProperty("Priority", "1");
    this->RecordProperty("DerivationTechnique", "Analysis of requirements");
...
TYPED_TEST(ProxyEventTracingSetReceiveHandlerFixture, SetReceiveHandlerCallsAreTracedWhenEnabled)
{
    ...
    this->RecordProperty("Description",
                         ...
                         "(8). The SetReceiveHandler trace points are traced without a LocalDataChunkList "
                         "(1, 5).");
    this->RecordProperty("TestType", "Requirements-based test");
    this->RecordProperty("Priority", "1");
    this->RecordProperty("DerivationTechnique", "Analysis of requirements");
...
TYPED_TEST(ProxyEventTracingReceiveHandlerCallbackFixture, ReceiveHandlerCallbackCallsAreTracedWhenEnabled)
{
    ...
    this->RecordProperty("Description",
                         ...
                         "(8). The ReceiveHandlerCallback trace points are traced without a "
                         "LocalDataChunkList (1, 5).");
    this->RecordProperty("TestType", "Requirements-based test");
    this->RecordProperty("Priority", "1");
    this->RecordProperty("DerivationTechnique", "Analysis of requirements");
    ...
    BindingEventReceiveHandler binding_event_receive_handler{};
    EXPECT_CALL(*this->mock_proxy_event_binding_, SetReceiveHandler(_))
        .WillOnce(Invoke([&binding_event_receive_handler](BindingEventReceiveHandler handler) -> ResultBlank {
            binding_event_receive_handler = std::move(handler);
            return {};
        }));
...
TYPED_TEST(ProxyEventTracingUnsetReceiveHandlerFixture, UnsetReceiveHandlerCallsAreTracedWhenEnabled)
{
    ...
    this->RecordProperty("Description",
                         ...
                         "(8). The UnsetReceiveHandler trace points are traced without a LocalDataChunkList "
                         "(1, 5).");
    this->RecordProperty("TestType", "Requirements-based test");
    this->RecordProperty("Priority", "1");
    this->RecordProperty("DerivationTechnique", "Analysis of requirements");
...
TYPED_TEST(ProxyEventTracingGetNewSamplesFixture, GetNewSamplesCallsAreTracedWhenEnabled)
{
    ...
    this->RecordProperty("Description",
                         ...
                         "(8). The GetNewSamples trace points are traced without a LocalDataChunkList "
                         "(1, 5).");
    this->RecordProperty("TestType", "Requirements-based test");
    this->RecordProperty("Priority", "1");
    this->RecordProperty("DerivationTechnique", "Analysis of requirements");
...
{
    this->RecordProperty("Verifies", "8, 1, 5, 7");
    this->RecordProperty(
        "Description",
        "The Trace point types for ProxyEvent/ProxyField GetNewSamplesCallback are correctly mapped "
        "(8). The GetNewSamplesCallback trace points are traced without a "
        "LocalDataChunkList (1, 5). The GetNewSamplesCallback trace points are traced with a "
        "trace_point_data_id.");
    this->RecordProperty("TestType", "Requirements-based test");
    this->RecordProperty("Priority", "1");
    this->RecordProperty("DerivationTechnique", "Analysis of requirements");
    ...
    amp::callback<void(SamplePtr<TestSampleType>)> callback = [](SamplePtr<TestSampleType>) noexcept {};
