...
TEST(ElementFqId, DefaultConstruction)
{
    ...
    EXPECT_EQ(fqid.element_id_, std::numeric_limits<std::uint8_t>::max());
...
TEST(ElementFqId, ConstructingEvent)
{
    ...
    const std::uint8_t element_id{11U};
...
TEST(ElementFqId, HashAcceptableCollisions)
{
    ...
    std::uniform_int_distribution<std::mt19937::result_type> distribUint8(1, std::numeric_limits<std::uint8_t>::max());
    ...
    for (size_t& hashval : hash_values)
    {
        hashval = hash(ElementFqId{static_cast<uint16_t>(distribUint16(rng)),
                                   static_cast<uint8_t>(distribUint8(rng)),
...
TEST(ElementFqId, InsertionOperator_DoesNotCrash)
{
    ...
    ::bmw::mw::log::LogDebug("lola") << element_fq_id;
}
