...
class TransactionLogRollbackExecutorFixture : public ::testing::Test
{
  protected:
    ...
    void InsertServiceDataControl(RollbackData& rollback_data) noexcept
    {
        auto rollback_data_it = rollback_data.synchronisation_data_set.insert(&service_data_control_);
        ASSERT_TRUE(rollback_data_it.second);
    }
...
TEST_F(TransactionLogRollbackExecutorRollbackLogsFixture, WillDoNothingWhenServiceDataControlAlreadyInSet)
{
    ...
    // already registered within the global RollbackData
    InsertServiceDataControl(rollback_data);
    EXPECT_CALL(lola_runtime_mock_, GetRollbackData()).WillOnce(ReturnRef(rollback_data));
...
TEST_F(TransactionLogRollbackExecutorRollbackLogsFixture,
       WillCreateRollbackDataEntryWhenRollbackDataDoesNotAlreadyExist)
{
    RollbackData rollback_data{};
    EXPECT_CALL(lola_runtime_mock_, GetRollbackData()).WillOnce(ReturnRef(rollback_data));

    EXPECT_EQ(rollback_data.synchronisation_data_set.count(&service_data_control_), 0);
    TransactionLogRollbackExecutor unit{
        &service_data_control_, kDummyQualityType, kDummyProviderPid, kDummyTransactionLogId};
    unit.RollbackTransactionLogs();
    EXPECT_EQ(rollback_data.synchronisation_data_set.count(&service_data_control_), 1);
}
...
TEST_F(TransactionLogRollbackExecutorRollbackLogsFixture, TryMarkTransactionLogsNeedRollbackCanBeCalledMultiThreaded)
{
    RollbackData rollback_data{};
    EXPECT_CALL(lola_runtime_mock_, GetRollbackData()).Times(kMaxSubscribers).WillRepeatedly(ReturnRef(rollback_data));
    ...
    for (std::size_t i = 0; i < kMaxSubscribers; ++i)
    {
        threads.emplace_back([&unit]() noexcept { unit.RollbackTransactionLogs(); });
    }
...
{
    RollbackData rollback_data{};
    EXPECT_CALL(lola_runtime_mock_, GetRollbackData()).WillRepeatedly(ReturnRef(rollback_data));
...
{
    // Given an empty/default constructed RollbackData and a LoLa runtime always returning it
    RollbackData rollback_data{};
    EXPECT_CALL(lola_runtime_mock_, GetRollbackData()).WillRepeatedly(ReturnRef(rollback_data));
